...
This paper investigates the long-term memory and multifractal properties of financial time series through advanced Hurst
...
Memory diagnostics in finance revolve around RS and its modified M-RS tests; Mandelbrot and Wallis pioneered the use
...
\begin{enumerate}
    \item Calculate the rolling Hurst exponent $H_{\text{rolling}}$ using a window size of 120 days via the Modified R/S method.
...
Figure~\ref{fig:perf_without_inef}. The number of position taken is reduced with our index (353 vs 443), which showcases that our inefficiency
index serves as a filter for the Hurst signal, reducing the number of false signals and improving the overall performance of the strategy.
...
all orders. This index can serve as a filter of the Hurst signal to reduce the number of positions taken and improving performance.
...
persistent temporal dependence, whereas the Russell 2000 exhibits genuine long-term memory. Consequently, the width of
the multifractal spectrum serves as a gauge of market inefficiency: the larger $\Delta \alpha$ for a surrogate series, the more the
process departs from ideal randomness by containing distinct local regimes. To come back where you left off, see Section~\ref{sec:multifractal_spectrum}.
