...
\begin{itemize}
    ...
    \begin{equation}
        \mathbb{E} \left[ (X_H(t) - X_H(s))^2 \right] = |t - s|^{2H},
    \end{equation}
    where \( H \) is the Hurst exponent, and \( \sigma^2 = 1 \).
...
\begin{equation}
    C_H(t_1, t_2) = \frac{1}{2} \left( t_1^{2H} + t_2^{2H} - |t_1 - t_2|^{2H} \right),
    \label{eq:fbm_covariance}
\end{equation}

which accounts for the dependence structure of the process, with \( \sigma^2 = 1 \). The Hurst exponent \( H \) plays a critical role in determining the smoothness and correlation properties of fBm:
