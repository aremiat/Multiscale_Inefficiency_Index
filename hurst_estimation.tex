...
\subsection{MF-DFA Methodology}

The Multifractal Detrended Fluctuation Analysis (MF-DFA) is a generalization of the standard DFA approach designed to detect multifractality in time series. The procedure can be summarized in five steps, as described below \cite{kantelhardt, barabasi}:
...
\begin{itemize}
    \item For \emph{monofractal} series, $h(q)$ is approximately constant for tous $q$.
    \item Pour les séries \emph{multifractales}, $h(q)$ dépend fortement de $q$, révélant une hétérogénéité de l'échelle des fluctuations.
\end{itemize}

\vspace*{1em}
\noindent
\textbf{Connection with the Modified R/S Analysis.} \\
The MF-DFA and the \emph{Modified R/S} analysis share a common objective: detecting the presence of \emph{long memory} and \emph{multifractality} in a time series. The Modified R/S statistic,
\[
\tilde{Q}_T \;=\; \frac{R}{\hat{\sigma}_T(q)},
\]
includes in its denominator not only the variance but also the autocovariances, which helps isolate \emph{long memory} in potentially autocorrelated series \cite{lo, neweywest, andrews}. While the R/S (or Modified R/S) approach primarily focuses on testing for long memory (using the statistic
\[
V \;=\; \frac{\tilde{Q}_T}{\sqrt{T}}
\]
and critical values derived from a Brownian bridge), the MF-DFA generalizes the analysis by measuring the \emph{entire multifractal spectrum} (via $h(q)$ and the width $\Delta \alpha$ of the singularity spectrum). Thus, the two methods are complementary:
...
\section{Holder Exponent and Singularity Spectrum}
...
The Hölder exponent, denoted as $\alpha(q)$, characterizes the local regularity (or singularity strength) of a signal. For a moment order $q$, it is defined via the Legendre transform of the scaling function $h(q)$:
\begin{equation}
\alpha(q) = h(q) + q\, h'(q),
\end{equation}
