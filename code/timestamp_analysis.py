...
if __name__ == "__main__":
    ...
    #     mode='lines',
    #     name=f'Rolling Critical Montlhy'
    #     ))
    #
    # # rolling_periods = pd.concat([rolling_periods, pd.DataFrame({
    # #     f"Rolling Critical Monthly": [rolling_critical]
    # # })], ignore_index=False)
    #
    # fig.add_trace(go.Scatter(
    #     x=rolling_critical.index,
    ...
    rolling_and_price = pd.concat([p_ticker, rolling_critical], axis=1, join="inner")
    rolling_and_price.columns = ['Price', 'Critical Value']
    rolling_and_price.to_csv(f"{DATA_PATH}/rolling_and_price.csv")
...
#         p_ticker = p[TICKER].dropna()
#         p_ticker = p_ticker.loc[st: "2025-02-28"]
#         log_p = np.log(p_ticker)
#         r = log_p.diff().dropna()
#         r = r.resample("M").last().dropna()
