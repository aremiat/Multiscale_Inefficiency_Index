...
class ComputeMFDFA:
    ...
    @staticmethod
    def mfdfa_rolling_opti_shuffle(
            series: Union[pd.Series, Sequence[float]],
            window_size: int,
            q_list: Sequence[float],
            scales: Sequence[int],
        ...
        """
        Rolling MF‑DFA optimisée.
        Renvoie uniquement la contribution corrélée :
            Delta_alpha_corr = Delta_alpha_total - Delta_alpha_shuffled
        Un suivi de progression + ETA est affiché sur stdout.
        """
        ...
        num_win = N - window_size + 1
        if num_win <= 0:
            return pd.Series([], name="alpha_width_corr")

        # --------- Pré‑calculs invariants ----------
        q = np.asarray(q_list, dtype=float)
        scales_arr = np.asarray(scales, dtype=float)
        log_s = np.log(scales_arr)
        x_mean = log_s.mean()
        denom = ((log_s - x_mean) ** 2).sum()

        # Préallocation résultat
        alpha_width_corr = np.empty(num_win, dtype=float)
        ...
        for i in range(num_win):
            ...
            # === 1. Série brute ===
            Fq_tot = ComputeMFDFA.mfdfa(window, scales_arr, q, order)
            h_q_tot = ComputeMFDFA._hurst_slopes(log_s, x_mean, denom, Fq_tot)
            alpha_tot, _ = ComputeMFDFA.compute_alpha_falpha(q, h_q_tot)
            width_tot = alpha_tot.max() - alpha_tot.min()

            # === 2. Série shufflée (détruit les corrélations) ===
            window_shuf = np.random.permutation(window)
            Fq_shuf = ComputeMFDFA.mfdfa(window_shuf, scales_arr, q, order)
            h_q_shuf = ComputeMFDFA._hurst_slopes(log_s, x_mean, denom, Fq_shuf)
            alpha_shuf, _ = ComputeMFDFA.compute_alpha_falpha(q, h_q_shuf)
            width_shuf = alpha_shuf.max() - alpha_shuf.min()

            # === 3. Contribution corrélation ===
            alpha_width_corr[i] = width_tot - width_shuf
        ...
        print()  # passe à la ligne une fois terminé

        # --------- Séries pandas ----------
        out_idx = idx[window_size - 1: window_size - 1 + num_win]
        return pd.Series(alpha_width_corr, index=out_idx, name="alpha_width_corr")

    # --- Helper privé pour calculer les pentes h(q) vectorisées ---
    @staticmethod
    def _hurst_slopes(log_s, x_mean, denom, Fq_mat):
        """
        Calcule le vecteur h(q) (pentes) pour une matrice Fq (shape = (len(q), len(scales))).
        """
        logF = np.log(Fq_mat)
        y_mean = logF.mean(axis=1)
