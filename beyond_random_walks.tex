...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Generalized Hurst exponent $h(q)$ for the Russell 2000 returns. Values of q are equally spaced between -3 and 3.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Generalized Hurst exponent $h(q)$ for the S\&P 500 returns. Values of q are equally spaced between -3 and 3.
    The scale used are logly spaced between 10 and 500.}
\end{figure}
...
shuffled series exhibits a more uniform behavior with less multifractality. The surrogate version of the series is really narrow
...
The relationship of multifractality are non-linear, therefore the only way to quantifie for each of those sources is via the shuffled and the surrogate one.
...
shrinks to 0.2 when shuffling the series, indicating that the multifractality is driven by long-range correlations.
...
In an ideal random walk (i.i.d. Gaussian increments), the process is monofractal and the multifractal spectrum collapses to a single point its.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{img/price_and_inefficiency_surrogate_russell}
    \caption{Log–price trajectory of the Russell 2000.Bottom corresponding inefficiency index \(I=\Delta\alpha_{\text{surrogate}} * |H-0.5|\).}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{img/price_and_inefficiency_surrogate_sp500}
    \caption{Log–price trajectory of the S\&P 500. Bottom corresponding inefficiency index \(I=\Delta\alpha_{\text{surrogate}} * |H-0.5|\).}
...
\pgfplotstabletypeset[
  ...
  every head row/.style={before row=\hline, after row=\hline},
  every last row/.style={after row=\hline},
  % colonne 0 = vos labels
  columns/0/.style={
    column name={},
    string type
  },
  % colonnes numériques
  columns/GSPC/.style={column name=GSPC, fixed, precision=3},
  columns/RUT/.style={column name=RUT,  fixed, precision=3},
  columns//.style={column name=FTSE, fixed, precision=3},
  columns/4/.style={column name=N225, fixed, precision=3},
  columns/5/.style={column name=GDAXI, fixed, precision=3}
]{data/granger_pvalues_signed.csv}
  \caption{Matrice de corrélation — valeurs arrondies à trois décimales}
  \label{table:corr3dec}
\end{table}
...
\begin{table}[!h]
    ...
    \caption{1991-11-15 to 2025-02-28 | Window size 120 days. Traditional R/S method is used here
    To comeback where you left off, see Section~\ref{sec:backtest_results}.}
